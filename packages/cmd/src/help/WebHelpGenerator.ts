...
export class WebHelpGenerator {
     ...
     */
    constructor(webHelpParms: IWebHelpParms, webHelpDir: string) {
    ...
    public buildHelp(cmdResponse: IHandlerResponseApi) {
        ...
        if (this.mWebHelpParms.webHelpLogoImgPath) {
            fsExtra.copySync(this.mWebHelpParms.webHelpLogoImgPath, path.join(webHelpDir, "header-image.png"));
        }

        // Replace main.css with custom CSS file if it exists
        if (this.mWebHelpParms.webHelpCustomCssPath) {
            fsExtra.copySync(this.mWebHelpParms.webHelpCustomCssPath, path.join(webHelpDir, "css/main.css"));
        }

        // Sort all items in the command tree and remove duplicates
        const uniqueDefinitions: ICommandDefinition = this.mWebHelpParms.fullCommandTree;
        ...
        const rootCommandName: string = this.mWebHelpParms.rootCommandName;
...
`;
    ...
    private genCommandHelpPage(definition: ICommandDefinition, fullCommandName: string, docsDir: string, parentNode: IWebHelpTreeNode) {
        ...
        const helpGen = new DefaultHelpGenerator({ produceMarkdown: true, rootCommandName } as any,
            { commandDefinition: definition, fullCommandTree: this.mWebHelpParms.fullCommandTree });
        ...
        if (this.sanitizeHomeDir) {
            const homeDir = path.dirname(this.mWebHelpParms.defaultHome);
    ...
    private writeTreeData() {
        ...
        fs.writeFileSync(treeDataPath,
            ...
            `const headerStr = "${this.mWebHelpParms.productDisplayName}";\n` +
            `const footerStr = "${this.mWebHelpParms.callerPackageJson.name} ${this.mWebHelpParms.callerPackageJson.version}";\n` +
