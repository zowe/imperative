...
import { IDeleteProfile, ILoadProfile, IProfileDeleted, IProfileLoaded } from "../../../profiles/src/doc";
...
export class CliProfileManager extends BasicProfileManager<ICommandProfileTypeConfiguration> {
  ...
  protected async saveProfile(parms: ISaveProfileFromCliArgs): Promise<IProfileSaved> {
    ...
    if (!isNullOrUndefined(parms.args)) {
      ...
      // profile should already be ready// profile should already be ready
  ...
  protected async updateProfile(parms: IUpdateProfileFromCliArgs): Promise<IProfileUpdated> {
    ...
    let updated: any;
    if (!isNullOrUndefined(parms.args)) {
      const loadedProfile = await new CliProfileManager(this.managerParameters).loadProfile({ name: parms.name });
  ...
  protected async loadProfile(parms: ICliILoadProfile): Promise<IProfileLoaded> {
    ...
    if (!parms.noSecure) {
      ...
      const securelyLoadValue: SecureOperationFunction = async (propertyNamePath: string): Promise<any> => {
        let ret;
        try {
          this.log.debug(`Loading secured field with key ${propertyNamePath} for profile ("${profile.name}" of type "${this.profileType}").`);
  ...
  protected async deleteProfile(parms: IDeleteProfile): Promise<IProfileDeleted> {
     ...
     * @return {Promise<string>}
     */
    const deleteSecureProperty: SecureOperationFunction = async (propertyNamePath: string): Promise<void> => {
      try {
        this.log.debug(`Deleting secured field with key ${propertyNamePath} for profile ("${parms.name}" of type "${this.profileType}").`);
        // Use the Credential Manager to store the credentials
        ...
        this.log.error(`Unable to delete secure field "${propertyNamePath}" associated with profile "${parms.name}" of type "${this.profileType}".`);
        this.log.error(`Error: ${err.message}`);
        throw new ImperativeError({
  ...
  private async processSecureProperties(profile: IProfile): Promise<IProfile> {
     ...
     * @return {Promise<string>}
     */
    const securelyStoreValue: SecureOperationFunction = async (propertyNamePath: string, propertyValue: string): Promise<string> => {
      if (isNullOrUndefined(propertyValue)) { // prevents from storing null values
        return null;
      }

      try {
        this.log.debug(`Associating secured field with key ${propertyNamePath} for profile ("${profile.name}" of type "${this.profileType}").`);
        // Use the Credential Manager to store the credentials
        ...
        this.log.error(`Unable to store secure field "${propertyNamePath}" associated with profile "${profile.name}" of type "${this.profileType}".`);
        this.log.error(`Error: ${err.message}`);
        throw new ImperativeError({
