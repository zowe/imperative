...
describe("Profile Create Command Builder", () => {
    ...
    it("should provide a valid command definition for the " +
        "profile create command based on our test profile type", () => {
        const firstProfileType = testBuilderProfiles[0];
        let commands = new ProfilesCreateCommandBuilder(firstProfileType.type, logger, firstProfileType).buildFull();
        commands = deleteHandlerPaths(commands);
        expect(commands).toMatchSnapshot();
    });

    it("should expose options for nested properties", () => {
        const command = new ProfilesCreateCommandBuilder("test",
            logger, {
                type: "test",
                schema: {
