...
Imperative.init({
    ...
    try {
         ...
         */
        cmdResp.console.log(Buffer.from(WRITE_MESSAGE_ONE));
        cmdResp.console.log(Buffer.from(WRITE_MESSAGE_TWO));
        /**
         * Messages should go to stderr with newlines appended automatically
         */
        cmdResp.console.error(Buffer.from(WRITE_ERR_MESSAGE_ONE));
        cmdResp.console.error(Buffer.from(WRITE_ERR_MESSAGE_TWO));
