...
export abstract class AbstractProfileManager<T extends IProfileTypeConfiguration> {
   ...
   * @memberof AbstractProfileManager
   */
  private static mLoadCounter: Map<string, number> = new Map<string, number>();
  ...
  protected get loadCounter(): Map<string, number> {
    return AbstractProfileManager.mLoadCounter;
  }
  ...
  public async load<L extends ILoadProfile>(parms: L): Promise<IProfileLoaded> {
    ...
    this.loadCounter.set(mapKey, (isNullOrUndefined(count) ? 1 : count++));
    this.log.debug(`Load count for "type_name" key "${mapKey}" is ${count || 1}`);
    if (count >= 2) {
