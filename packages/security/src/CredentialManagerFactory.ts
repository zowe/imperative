  ...
  public static async initialize(Manager: IImperativeOverrides["CredentialManager"], cliName: string): Promise<void> {
    ...
    if (typeof Manager === "string") {
      ...
      try {
        ...
        tempManager = new LoadedManager(cliName);
      } catch (e) {
        throw new ImperativeError({
          msg: `Unexpected error while trying to load ${Manager}`,
          additionalDetails: e.message,
          causeErrors: e
        });
      }

      // After we load and call the constructor, we will validate that the thing we imported does in fact extend the
      // AbstractCredentialManager. If it doesn't we will throw an error.
      if (tempManager instanceof AbstractCredentialManager) {
        manager = tempManager;
      } else {
        throw new ImperativeError({
          msg: `The manager provided at ${Manager} does not extend AbstractCredentialManager properly!`
        });
      }
    } else {
    ...
    // Otherwise we should initialize the credential manager.
