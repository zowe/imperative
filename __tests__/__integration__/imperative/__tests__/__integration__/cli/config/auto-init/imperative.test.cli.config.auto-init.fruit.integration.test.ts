...
describe("cmd-cli config auto-init", () => {
    ...
    it("should initialize project config", async () => {
        ...
        expect(configJson.profiles.base_fruit.properties.authToken).toBeUndefined();
        expect(configJson.profiles.base_fruit.secure).toEqual(["authToken"]);
        expect(await loadSecureProp("base_fruit")).toBe("jwtToken=admin:123456@fakeToken");
    });
    ...
    it("should prompt for missing user", async () => {
        ...
        expect(configJson.profiles.base_fruit.properties.authToken).toBeUndefined();
        expect(configJson.profiles.base_fruit.secure).toEqual(["authToken"]);
        expect(await loadSecureProp("base_fruit")).toBe("jwtToken=fakeInput:123456@fakeToken");
    });
    ...
    it("should authenticate with token if provided", async () => {
        ...
        expect(configJson.profiles.base_fruit.properties.authToken).toBeUndefined();
        expect(configJson.profiles.base_fruit.secure).toEqual(["authToken"]);
        expect(await loadSecureProp("base_fruit")).toBe("apimlAuthenticationToken=null");
    });
