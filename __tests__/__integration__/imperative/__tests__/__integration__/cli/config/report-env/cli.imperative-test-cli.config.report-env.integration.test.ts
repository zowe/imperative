...
import { EnvFileUtils } from "../../../../../../../../packages";
...
describe("imperative-test-cli config report-env", () => {
    ...
    describe("success scenarios", () => {
        ...
        it("should set up environment with file and successfully produce a report", async () => {
            const pluginsDir = path.join(TEST_ENVIRONMENT.workingDir, "plugins");
            if (!fs.existsSync(pluginsDir)) {
                fs.mkdirSync(pluginsDir);
            }
            ...
            expect(response.stdout.toString()).toContain("ZOWE_OPT_TEST =");
